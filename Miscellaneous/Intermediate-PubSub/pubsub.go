// Implementation taken from github.com/cskr/pubsub
package pubsub
...
// Sub return a channel, on which messages published on any of the specified topics can be received
...
// Unsub unsubscribes the given channel form the specified topics. If no topic is specified, it is unsubscribed from all topics.
//
// Unsub must bed called from a goroutine that is different from the subscriber. The subscriber must consume messages from the channel untill it reaches the end. Not doing so might result in a deadlock
...
// Close closes all channels currently subscried to the specified topics. If a channel is subscribed to multiple topics, some of which is not specified, it is not closed.
...
// registry maintains the current subscription state. It's not safe to access a registry from multiple goroutines simultaneosly
